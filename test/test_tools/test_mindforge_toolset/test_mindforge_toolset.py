...
class MindforgeToolsetTestCase(unittest.TestCase):
    def setUp(self):
        self.tool = MindforgeToolset(auto_install=False)
    ...
    def test_call_tool_function_should_ignore_unexpected_kwargs(self):
        original_get_tool_instance = self.tool._get_tool_instance
        self.tool._get_tool_instance = lambda toolset_key: _FakeCreateTool()
        try:
            result = self.tool.call_tool_function(
            ...
            self.tool._get_tool_instance = original_get_tool_instance
