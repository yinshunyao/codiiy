...
class ReActEngineTestCase(unittest.TestCase):
    ...
    def test_run_completes_with_tool_then_final_answer(self):
        ...
        self.assertEqual(result.final_answer, "处理完成")
    ...
    def test_run_records_unknown_tool_error_and_continue(self):
        ...
        model_outputs = [
            ...
            ('{"thought":"直接结束","final_answer":"兜底完成"}', ""),
        ]
        ...
        self.assertTrue(result.success)
        self.assertEqual(result.final_answer, "兜底完成")
        self.assertEqual(len(result.steps), 2)
