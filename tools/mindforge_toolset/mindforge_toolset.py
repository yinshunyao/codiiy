...
class MindforgeToolset:
    """mindforge 专用工具集代理，支持 tools/component 统一调用入口。"""

    _MODULE_PATH = "tools.mindforge_toolset.mindforge_toolset"
    ...
    def control_call(
        ...
        """统一调用入口：优先执行 tools.*，仅保留少量内部 component 调用。"""
        try:
            assert_toolset_enabled(self._MODULE_PATH)
