...
### `create_tool(folder_name: str, init_content: str, readme_content: str, code_file_name: str, code_content: str, overwrite: bool = False) -> Dict`

| 参数 | 类型 | 默认值 | 说明 |
|:---|:---|:---|:---|
| `folder_name` | `str` | 必填 | 工具目录名，仅允许字母/数字/下划线，且不能以数字开头。 |
| `init_content` | `str` | 必填 | `__init__.py` 完整内容。 |
| `readme_content` | `str` | 必填 | `README.md` 完整内容。 |
| `code_file_name` | `str` | 必填 | 主代码文件名，必须是单个 `.py` 文件名。 |
| `code_content` | `str` | 必填 | 主代码文件完整内容。 |
| `overwrite` | `bool` | `False` | 文件已存在时是否覆盖。 |

返回 `data` 字段：`tool_name`、`tool_dir`、`written_files`、`source`。
